...
// Fetch schemes with filters (for filter button)
...
// Only ONE DOMContentLoaded block for all initialization
document.addEventListener('DOMContentLoaded', function() {
  // Attach filter button event listener
  const filterBtn = document.getElementById('filter-btn');
  if (filterBtn) {
    filterBtn.addEventListener('click', fetchSchemesWithFilters);
  }

  // Attach clear filters button event listener
  ...
  // Attach category, district, level filter change listeners
  ...
  // Check if a category was selected from index.html
  const selectedCategory = localStorage.getItem('selectedCategory');
  if (selectedCategory && categoryFilter) {
    categoryFilter.value = selectedCategory;
    localStorage.removeItem('selectedCategory');
    applyFilters(); // Automatically apply the filter
  } else {
    loadSchemes();
  }
});